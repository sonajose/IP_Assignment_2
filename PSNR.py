...
cv2.imwrite('dummy.jpg' , inv_deblur)


#------------------------------------PSNR---------------------------------------

max_sq = 255*255
mse1 = np.sum(np.square(original - image_in))/(A*B)
mse2 = np.sum(np.square(original - inv_deblur))/(A*B)
#Mean square error
psnr1 = 10*np.log10(max_sq/mse1)
psnr2 = 10*np.log10(max_sq/mse2)
print(psnr1)
print(psnr2)
