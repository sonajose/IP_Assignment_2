...
def butterworth(order, rad):
     ...
     butter_lpf = np.divide(np.ones_like(r), np.power(r + c, order * np.ones_like(r)) + np.ones_like(r))

     return butter_lpf

lpf = np.fft.fftshift(butterworth(12, R)) #Butterworth low pass filter of order 12 and radius R
...
for i in range(0, 3):
    ...
    lap_zeropad = np.zeros_like(image_in[:, :, 0])
    lap_zeropad[0:3, 0:3] = laplace_matrix
    lap_zeropad = lap_zeropad 

    laplacian_operator = np.fft.fft2(laplace_matrix) #Taking dft of laplacian matrix
